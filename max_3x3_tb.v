...
module max_3x3_tb();
    ...
    wire output_rdy; 
    reg [31:0] matrixA [0:2][0:2];
    reg [31:0] matrixB [0:2][0:2];
    reg [31:0] matrixResult [0:2][0:2];

    wire [95:0] rowResult_flat;
    wire [31:0] rowResult [0:2];
    ...
    // unflattening result
    assign rowResult[0] = rowResult_flat[95:64];
    ...
    always @(posedge clk) begin
        ...
        if(output_rdy) begin
            for (j = 0; j < 3; j = j + 1) begin : assign_row
    ...
    wire [95:0] flat_A, flat_B;
    assign flat_A = {inA[0], inA[1], inA[2]};
    assign flat_B = {inB[0], inB[1], inB[2]};
    max_3x3 matmultest(.clk(clk), .rst(rst), .input_start(input_start), .output_rdy(output_rdy), .outD_flat(rowResult_flat), .inA_flat(flat_A), .inB_flat(flat_B), .counter(counter));
    ...
    initial begin
        // output file name
        $dumpfile("max_3x3.vcd");
        // module to capture and what level, 0 means all wires
        $dumpvars(0, max_3x3_tb);
    end
